...
var REQUEST = function REQUEST(type) {
  return function () {
    ...
    // Return the action
    return Object.assign({ type: type }, config.meta && { meta: config.meta });
  };
};
...
var SUCCESS = function SUCCESS(type) {
  return function () {
    ...
    // Return the action
    return Object.assign({ type: type }, (normalized || data) && { payload: normalized || data }, (response || config.meta) && { meta: Object.assign({}, config.meta, response && { response: response }) });
  };
};
 ...
 * @param {string} type - Action type
 */
var FAILURE = function FAILURE(type) {
  return function () {
    ...
    return Object.assign({ type: type, error: true }, error && { payload: error }, (config.meta || error && error.response) && { meta: Object.assign({}, error && error.response && { response: error.response }, config.meta) });
  };
};

module.exports = { REQUEST: REQUEST, SUCCESS: SUCCESS, FAILURE: FAILURE };